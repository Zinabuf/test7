...
#' @export
qqp_gweis <- function(plink_path, dis_snp, qp_dis_phen, qp_dis_cov) {
temp_dir <- tempdir()
  # Run PLINK for GWEIS
  system(paste(
    ...
    "--out", file.path(temp_dir, "qqp_gweis"), "&> plink.log"
  ))

  # Process GWEIS results
  gweis_result <- read.table(file.path(temp_dir, "qqp_gweis.PHENO1.glm.linear"),
