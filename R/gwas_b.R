...
#' @export
bp_gwas <- function(plink_path, dis_snp, bp_dis_cov) {
  temp_dir <- tempdir()
...
system(paste(
  ...
  "--out", file.path(temp_dir, "bp_gwas"), "&> gwas.log"
))
  covariate_result <- read.table(file.path(temp_dir, "bp_gwas.PHENO1.glm.logistic.hybrid"),
