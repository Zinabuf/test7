...
gcim_q <- function(qp_tar_phen, qp_tar_cov, Add_PRS, Int_PRS, Cov_PRS, confounders) {
  ...
  outcome_bp_data <- read.table("qp_tar_phen", header = TRUE, stringsAsFactors = FALSE)
  ...
  covariate_bp_data <- read.table("qp_tar_cov", header = TRUE, stringsAsFactors = FALSE, fill = TRUE)
